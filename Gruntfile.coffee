...
module.exports = ( grunt ) ->
	...
	buildScssHead =
		expand : yes
		flatten: yes
		src    : [ '<%= pkg.buildDir %>/<%= pkg.almostBuiltDir %>/<%= pkg.scssDir %>/**/!(_)*.scss' ]
		dest   : '<%= pkg.buildDir %>/<%= pkg.builtDir %>/<%= pkg.cssDir %>/'
		ext    : '.css'
		extDot : 'last'
	...
	grunt.initConfig
		...
		# # This is no longer used technically (we instead cache the generated file we need in ./modernizr.head.js), but is still here for convenience and for the record (of what we actually use and therefore need)
		# modernizr:
			...
			development:
				...
				options:
					...
					styles:
						...
						headIE: [ allCssIE ]
					sections:
						head: "#{ packageJson.buildDir }/#{ packageJson.builtDir }/0_head.html"
						tail: "#{ packageJson.buildDir }/#{ packageJson.builtDir }/0_tail.html"
	...
	#                                                                               , 'modernizr'
	grunt.registerTask 'doCoffeeDevelopment', doCoffeeDevelopment = [ 'clean:coffee', 'preprocess:prepCoffee', 'preprocess:prepJs', 'preprocess:prepTemplates', 'copy:buildJs', 'copy:buildTemplates', 'coffee', 'ngtemplates', 'concat:prettyHead', 'concat:prettyTail', 'concat:allPrettyJs',                  'htmlbuild:development' ]
